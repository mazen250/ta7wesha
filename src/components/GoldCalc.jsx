...
export default function GoldCalc({ rates }) {
  ...
  return (
    <div className="rounded-2xl bg-[var(--c-card)] border border-[var(--c-border)] p-5" role="region" aria-label="Gold calculator">
      ...
      <div className="space-y-3">
        ...
        {grams > 0 && (
          <div className="pt-2 border-t border-[var(--c-border)]">
            <p className="text-[10px] font-bold text-[var(--c-t3)] uppercase tracking-[0.1em] mb-1">Melt Value</p>
            <p className="text-xl font-extrabold text-amber-400 tracking-tight">
              {formatNumber(meltValue, 2)} <span className="text-xs font-bold text-[var(--c-t3)]">EGP</span>
            </p>
            <p className="text-[11px] font-semibold text-[var(--c-t3)] mt-0.5">
              ${formatNumber(meltValue / rates.usdToEgp, 2)} USD
            </p>
            <p className="text-[10px] text-[var(--c-t4)] mt-1">
              {karat}K price/gram: EGP {formatNumber(pricePerGram, 2)}
            </p>
          </div>
        )}
