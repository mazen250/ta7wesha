...
export default function NetWorthChart({ breakdown }) {
  ...
  return (
    <div className="rounded-2xl bg-[var(--c-card)] border border-[var(--c-border)] p-5" role="region" aria-label={t('netWorthBreakdown')}>
      ...
      <div className="flex items-center gap-6">
        <svg width="140" height="140" viewBox="0 0 140 140" className="shrink-0" role="img">
          {breakdown.map(segment => {
            ...
            return (
              <circle key={segment.key} cx="70" cy="70" r={RADIUS} fill="none" stroke={segment.color} strokeWidth="16" strokeDasharray={`${segmentLength} ${CIRCUMFERENCE - segmentLength}`} strokeDashoffset={offset} strokeLinecap="round" transform="rotate(-90 70 70)" className="transition-all duration-700" />
            );
          })}
          <text x="70" y="66" textAnchor="middle" className="fill-[var(--c-t1)] text-xs font-bold">{breakdown.length}</text>
          <text x="70" y="80" textAnchor="middle" className="fill-[var(--c-t3)] text-[10px] font-medium">{t('assets')}</text>
        </svg>
          ...
          {breakdown.map(segment => (
            <div key={segment.key} className="flex items-center gap-2.5">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-[10px] font-medium text-[var(--c-t3)]">{t('egp')} {formatNumber(segment.value)}</p>
              </div>
            </div>
          ))}
        </div>
